...
import { AddFormRecipeSchema, AddFormRecipeType } from '../../schemas/recipe'
...
const AddRecipe = () => {
	...
	const {
		...
		defaultValues: {
			...
			cookDuration: 0,
			prepDuration: 0,
			ingredients: [{ name: '', unit: 'gram', quantity: 0 }],
		},
		resolver: zodResolver(AddFormRecipeSchema),
	})

	const { fields, append, remove } = useFieldArray<AddFormRecipeType>({
		control,
		name: 'ingredients',
	})
	...
	const onAddRecipe: SubmitHandler<AddFormRecipeType> = data => {
		addRecipe(data)
		reset()
		navigate('/')
	}
	...
	return (
		<Box
			...
			<Paper sx={{ p: 3, maxWidth: '500px', width: '100%' }}>
				...
				<Stack component={'form'} onSubmit={handleSubmit(onAddRecipe)} noValidate autoComplete='off' spacing={2}>
					<TextField
						label='Recipe Title'
						size='small'
						{...register('title')}
					...
					<Box>
								...
								render={({ field: { ref, name, onBlur, onChange } }) => (
									<VisuallyHiddenInput
										...
										onChange={e => {
											...
											trigger('image')
											getBase64(getValues('image'), result => {
						...
						{
							// Task
							!errors.image && getValues('image') instanceof File ? (
